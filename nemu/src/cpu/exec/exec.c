...
opcode_entry opcode_table [512] = {
  ...
  /* 0x04 */	IDEXW(I2a,add,), IDEX(I2a,add), EMPTY, EMPTY,
  ...
  /* 0xf0 */	EMPTY, EX(cheat), EX(cheat2), EMPTY,
  ...
  /* 0x1c */	EMPTY, EMPTY, EMPTY, EMPTY,
  /* 0x20 */	IDEXW(CR2R, mov_cr2r, 4), EMPTY, IDEXW(R2CR, mov_r2cr, 4), EMPTY,
...
void exec_wrapper(bool print_flag) {
  ...
  if (print_flag) {
	...
	for(i=0;i<sizeof(watchregs)/4;++i){
					...
					printf("%s:%4d(0x%4x)\t",regsl[watchregs[i]],cpu.gpr[watchregs[i]]._32,cpu.gpr[watchregs[i]]._32);
	}
	for(i=0;i<sizeof(watchaddrs)/4;++i){
					if(!(i&3)){
									puts("");
					}
